...
parser.add_argument('--training_face_rect_dir', default='../../data/train', help='Training dataset directory')
...
parser.add_argument('--resume_net', default='weights/facemask.pth', help='resume net for retraining')
...
parser.add_argument('--weight_filename_prefix', default='weights/yunetrgb', help='the prefix of the weight filename')
args = parser.parse_args()


img_dim = 320 # only 1024 is supported
rgb_mean = (104, 117, 123) # (0,0,0)  bgr order
num_classes = 3
...
if args.resume_net is not None:
    ...
    state_dict = torch.load(args.resume_net, map_location=lambda storage, loc: storage)
...
def train():
    ...
    dataset = FaceRectLMDataset(training_face_rect_dir, img_dim, rgb_mean)
    ...
    for epoch in range(args.resume_epoch, max_epoch):
        ...
        for iteration in range(epoch_size):
            ...
            if ( iteration % 10 == 0):
                print('Epoch:{}/{} || Epochiter: {}/{} || L: {:.2f}({:.2f}) C: {:.2f}({:.2f}) All: {:.2f}({:.2f}) || LR: {:.8f}'.format(epoch, max_epoch, iteration, epoch_size, loss_l.item(), mean_l_loss, loss_c.item(), mean_c_loss, loss.item(), mean_loss, lr))
